...
class HyperNetwork(nn.Module):
    ...
    def __init__(self, f_size = 3, z_dim = 16, out_size=64, in_size=64):
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if downsample is not None:
            self.hypernet_down = HyperNetwork(f_size = 1, in_size = inplanes, out_size = outplanes)
        ...
        self.bn_down = norm_layer(outplanes)
    ...
    def forward(self, x, inv):
        ...
        emb_ = self.emb1(inv)
        # Conv 1
        w1 = self.hypernet1(emb_) # weights
        x = F.conv2d(x, w1)
        x = self.bn1(x)
        x = self.act1(x)

        # Conv 2
        w2 = self.hypernet2(emb_)
        x = F.conv2d(x, w2, stride = 1 if self.use_aa else self.stride, padding=self.first_dilation, dilation=self.first_dilation, groups=self.cardinality)
        x = self.bn2(x)
        x = self.drop_block(x)
        x = self.act2(x)
        x = self.aa(x)
        ...
        if self.downsample is not None:
            # Downsampling conv
            w_down = self.hypernet_down(emb_)
            shortcut = self.downsample[1](self.downsample[0](shortcut, w_down))
        x += shortcut
        x = self.act3(x)
...
# MOdified for a hyper version
class DownSampleConv(nn.Module):
    def __init__(self, stride, padding, first_dilation):
        super().__init__()
        self.stride = stride
        self.padding = padding
        self.first_dilation = first_dilation
    
    def forward(self, x, w_down):
        return F.conv2d(x, w_down, stride = self.stride, padding = self.padding, dilation = self.first_dilation)
...
def downsample_conv(
    ...
    return nn.Sequential(*[
        DownSampleConv(
            stride=stride, padding=p, first_dilation=first_dilation),
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if deep_stem:
            ...
            # self.conv1_hn = HyperNetwork(f_size = 7, in_size = in_chans, out_size = inplanes)
            # self.emb_conv1 = nn.Linear(2, 16)
    ...
    def forward_features(self, x, inv):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # Every layer is wrapped in nn.Sequential which will not take inv as input. Therefore, we need to iterate over BottleNeck blocks in the layers

            for i in range(0, len(self.layer1)):
                x = self.layer1[i](x, inv)
            
            for i in range(0, len(self.layer2)):
                x = self.layer2[i](x, inv)
            
            for i in range(0, len(self.layer3)):
                x = self.layer3[i](x, inv)
            
            for i in range(0, len(self.layer4)):
                x = self.layer4[i](x, inv)

            return x
        

    def forward_head(self, x, inv_index = None, pre_logits: bool = False):
